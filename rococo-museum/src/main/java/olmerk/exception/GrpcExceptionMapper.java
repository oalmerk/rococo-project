...
public final class GrpcExceptionMapper {
    ...
    public static RuntimeException map(MuseumNotFoundException ex) {
        return Status.NOT_FOUND
                .withDescription(ex.getMessage())
                .asRuntimeException();
    }

    public static RuntimeException map(CountryNotFoundException ex) {
        return Status.NOT_FOUND
                .withDescription(ex.getMessage())
                .asRuntimeException();
    }

    public static RuntimeException map(DataIntegrityViolationException ex,
                                       String museumTitle) {

        if (isUniqueViolation(ex)) {
            return Status.ALREADY_EXISTS
                    .withDescription(
                            "Museum with title '" + museumTitle + "' already exists"
                    )
                    .asRuntimeException();
        }
